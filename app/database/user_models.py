...
if TYPE_CHECKING:
    # Type-only imports to avoid circular runtime imports
    ...
    from app.database.payments_models import Subscription  # type hint only
...
class User(Base):
    ...
    # Payments / Subscriptions relationship (type-hinted via TYPE_CHECKING)
...
class Profile(Base):
    ...
    salary_expectations_max: Mapped[Optional[int]] = mapped_column(Mapped[Optional[int]] if False else Integer, nullable=True, index=True)  # in thousands
